...
#define DISPATCH_DISCRETIZATION(D_METHOD, CONST_NAME, ...)                     \
    if (D_METHOD == DiscretizationMethod::ZOH) {                               \
        ...
        __VA_ARGS__();                                                         \
    }  else if (D_METHOD == DiscretizationMethod::MAMBA) {                     \
        constexpr auto CONST_NAME = DiscretizationMethod::MAMBA;               \
        __VA_ARGS__();                                                         \
    } else {                                                                   \
        /* S7 and RGLRU are real-only and dispatched before this macro. */      \
...
void set_ssm_params_fwd(SSMParamsBase &params,
    ...
    params.deltaA_batch_stride = delta.stride(0);
    params.deltaA_d_stride = delta.stride(1);
    if (has_z) {
...
void set_ssm_params_bwd(SSMParamsBwd &params,
    ...
    set_ssm_params_fwd(params, batch, dim, seqlen, dstate, n_groups, n_chunks, is_variable_B, is_variable_C,
                       ...
                       D_ptr, delta_bias_ptr, deltaA_ptr, x_ptr, has_z, delta_softplus, d_method); 
...
std::vector<at::Tensor> selective_scan_fwd(const at::Tensor &u, const at::Tensor &delta,
    ...
    } else if(discretization_method_str == "mamba"){
        d_method = DiscretizationMethod::MAMBA;
    } else if(discretization_method_str == "s7"){
        d_method = DiscretizationMethod::S7;
    } else if(discretization_method_str == "rglru"){
        d_method = DiscretizationMethod::RGLRU;
    }
    else {
        TORCH_CHECK(false, "Discretization method not supported");
    }

    if (d_method == DiscretizationMethod::S7) {
        TORCH_CHECK(weight_type == at::ScalarType::Float, "S7 discretization only supports real (float) weights, not complex");
    }

    if (d_method == DiscretizationMethod::RGLRU) {
        TORCH_CHECK(weight_type == at::ScalarType::Float, "RGLRU discretization only supports real (float) weights, not complex");
    }
    ...
    if (deltaA_.has_value()) {
        ...
        TORCH_CHECK(deltaA.scalar_type() == input_type);
    ...
    TORCH_CHECK(u.scalar_type() == at::kFloat, "Input tensor 'u' must be of type float32 for this kernel.");

    if (params.d_method == DiscretizationMethod::S7) {
        selective_scan_fwd_cuda<float, float, DiscretizationMethod::S7>(params, stream);
    } else if (params.d_method == DiscretizationMethod::RGLRU) {
        selective_scan_fwd_cuda<float, float, DiscretizationMethod::RGLRU>(params, stream);
    } else {
        DISPATCH_WTYPE_FLOAT_AND_COMPLEX(weight_type, selective_scan_fwd, [&] {
            DISPATCH_DISCRETIZATION(params.d_method, DMethod, [&] {
                selective_scan_fwd_cuda<float, weight_t, DMethod>(params, stream);
            });
        });
    }
...
std::vector<at::Tensor> selective_scan_bwd(const at::Tensor &u, const at::Tensor &delta,
    ...
    set_ssm_params_bwd(params, batch_size, dim, seqlen, dstate, n_groups, n_chunks, is_variable_B, is_variable_C,
                       ...
                       dout, du, ddelta, ddeltaA, dA, dB, dC, dz,
                       D_.has_value() ? dD.data_ptr() : nullptr,
                       delta_bias_.has_value() ? ddelta_bias.data_ptr() : nullptr,
                       has_z, has_deltaA, delta_softplus, recompute_out_z,
                       d_method);
    ...
    if (params.d_method == DiscretizationMethod::S7) {
        selective_scan_bwd_cuda<float, float, DiscretizationMethod::S7>(params, stream);
    } else if (params.d_method == DiscretizationMethod::RGLRU) {
        selective_scan_bwd_cuda<float, float, DiscretizationMethod::RGLRU>(params, stream);
    } else {
        DISPATCH_WTYPE_FLOAT_AND_COMPLEX(weight_type, selective_scan_bwd, [&] {
            DISPATCH_DISCRETIZATION(params.d_method, DMethod, [&] {
                selective_scan_bwd_cuda<float, weight_t, DMethod>(params, stream);
            });
        });
    }
