"""
Unit tests for RG-LRU (Recurrent Gated Linear Recurrent Unit).
"""
...
def test_rglru_step_consistency(device, seq_len, use_fast_path):
    """Test that step-by-step inference matches parallel forward pass."""
    if use_fast_path:
        pytest.skip("CUDA kernel not available for fast path")

    batch_size = 32
    d_model = 32

    model = RGLRU(d_model=d_model, use_fast_path=use_fast_path).to(device)
    model.eval()
...
@pytest.mark.parametrize("seq_len", SEQ_LENGTHS)
def test_rglru_cuda_vs_ref_forward(device, seq_len):
    """Test CUDA fast-path model matches reference (slow-path) model."""
    batch_size = 32
    d_model = 32

    model_cuda = RGLRU(d_model=d_model, use_fast_path=True).to(device)
    model_ref = RGLRU(d_model=d_model, use_fast_path=False).to(device)
    ...
    assert torch.allclose(y_cuda, y_ref, rtol=RTOL, atol=ATOL), (
        f"CUDA/Ref forward mismatch (L={seq_len}): "
...
@pytest.mark.parametrize("seq_len", SEQ_LENGTHS)
def test_rglru_gradient_consistency(device, seq_len):
