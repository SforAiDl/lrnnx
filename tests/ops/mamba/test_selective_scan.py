...
# @pytest.mark.parametrize("is_variable_B", [False])
def test_selective_scan(
    ...
    A = (
        -0.5 * torch.rand(dim, dstate, device=device, dtype=wtype)
    ).requires_grad_()
    ...
    delta_bias_ref = (
        delta_bias.detach().clone().requires_grad_()
...
@pytest.mark.parametrize("return_last_state", [True])
@pytest.mark.parametrize("has_D", [True])
def test_selective_scan_complex(
    has_D,
    return_last_state,
    seqlen,
    itype,
