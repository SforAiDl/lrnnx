"""
Unit tests for S4 (Structured State Space) layer.
"""
...
def test_s4_step_consistency(device, seq_len):
...
@pytest.mark.parametrize("seq_len", SEQ_LENGTHS)
def test_s4_fft_vs_recurrence(device, seq_len):
    ...
    step_mode = getattr(model.layer.kernel, "_step_mode", "dense")
    ...
    for b in range(batch_size):
        ...
        for t in range(seq_len):
            ...
            if step_mode == "linear":
                y_t = torch.einsum("chn,hn->ch", dC, state[..., :C_dim])
            else:
                y_t = torch.einsum("chn,hn->ch", dC, state[..., :C_dim])
            y_t = y_t.real
...
@pytest.mark.parametrize("seq_len", SEQ_LENGTHS)
def test_s4_gradient_consistency(device, seq_len):
    ...
    step_mode = getattr(model_rec.layer.kernel, "_step_mode", "dense")
    ...
    for (name_fft, param_fft), (name_rec, param_rec) in zip(
        ...
        # Skip if gradient is None in recurrence path (due to @torch.no_grad() in _setup_step)
        if grad_rec is None:
            continue
